...
class TrainModel:
    ...
    def _build_model(self, num_layers, width):
        ...
        backbone = tf.keras.Sequential(
            [
                Input((self._input_dim,)),
                Dense(480, activation="relu"),
                Dense(240, activation="relu"),
            ]
        )
        state_input = Input((self._input_dim,))
        backbone_1 = Dense(480, activation="relu") \
            (state_input)
        backbone_2 = Dense(240, activation="relu") \
            (backbone_1)
        value_output = Dense(1)(backbone_2)
        advantage_output = Dense(self._output_dim) \
            (backbone_2)
        output = Add()([value_output, advantage_output])
        model = tf.keras.Model(state_input, output, name='my_model')
