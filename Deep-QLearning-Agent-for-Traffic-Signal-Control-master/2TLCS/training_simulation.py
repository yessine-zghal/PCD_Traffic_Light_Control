...
class Simulation():
    def __init__(self, Model,Model2, mem,Buffer, TrafficGen, sumo_cmd, gamma, max_steps, green_duration,
                 yellow_duration, num_cells, num_states, num_actions, training_epochs):
        self._Model = Model
        self._Model2 = Model2
        self._mem2 = mem
        self._Buffer = Buffer
    ...
    def run(self, episode, epsilon):
        ...
        while self._step < self._max_steps:
            ...
            if self._step != 0:
                ...
                self._Buffer.add_sample((old_state_two, old_action_two, reward_two, current_state_two))

                self._mem2.add_sample((old_state_one, old_action_one, reward_one, current_state_one))
            ...
            action_one = self._choose_action(current_state_one, epsilon)
            # choose the light phase to activate, based on the current state of the second intersection
            action_two = self._choose_action(current_state_two, epsilon)
    ...
    def _choose_action(self, state, epsilon):
        ...
        if random.random() < epsilon:
            ...
            pred = self._Model.predict_one(state)
    ...
    def _get_density(self):
        ...
        """
        divider = traci.lane.getLength(
            "N2TL_0") / 1000  # derives the id of the first lane from the edge id (all lanes of an edge have the same length) and 1000 m -> km
    ...
    def _get_states_with_advanced_perception(self):
        ...
        state_one = np.concatenate((nb_cars[:self._num_cells], avg_speed[:self._num_cells],
                                    cumulated_waiting_time[:self._num_cells], nb_queued_cars[:self._num_cells]))
        state_two = np.concatenate((nb_cars[self._num_cells:], avg_speed[self._num_cells:],
                                    cumulated_waiting_time[self._num_cells:], nb_queued_cars[self._num_cells:]))
    ...
    def _replay(self):
        ...
        batch = self._mem2.get_samples(self._Model.batch_size)

        if len(batch) > 0:
            # if the memory is full enough
    ...
    def _replay2(self):
        ...
        batch = self._Buffer.get_samples(self._Model2.batch_size)
        ...
        if len(batch) > 0:  # if the memory is full enough
            ...
            self._model_training_loss.append(self._Model2.training_loss)  # get the MAE loss
    ...
    def _save_episode_stats(self):
        ...
        self._reward_store.append(
            self._sum_neg_reward_one + self._sum_neg_reward_two)  # how much negative reward in this episode for both agents
        ...
        self._avg_wait_time_per_vehicle.append(
            cum / self._sum_queue_length)  # how much time a vehicle wait in an episode
    ...
    # End simulation
    def stop(self):
        return self.reward_store[0], self._reward_store_a1[0], self._reward_store_a2[0], self.cumulative_wait_store[0], \
               self._cumulative_wait_store_a1[0], self._cumulative_wait_store_a2[0], self.avg_queue_length_store[0], \
               self._avg_queue_length_store_a1[0], self._avg_queue_length_store_a2[0], self.avg_wait_time_per_vehicle[
                   0], self._avg_wait_time_per_vehicle_a1[0], self._avg_wait_time_per_vehicle_a2[
                   0], self.density, self.flow, self.occupancy
